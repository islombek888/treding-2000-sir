...
import { type SignalData } from './alertService.js';

export class SignalEngine {
    /**
     * Processes new market data and determines if a signal should be triggered
     */
    static process(candles: Candle[]) {
        if (candles.length < 50) return; // Need enough data for EMAs
        ...
        if (evaluation.score >= 70 && state.trend !== MarketTrend.RANGING) {
            ...
            const signal: SignalData = {
                ...
                timeframe: '5m' // Default for legacy engine
            };
    ...
    private static generateReason(state: any, divergence: string): string {
        const reasons: string[] = [];
        if (state.trend === MarketTrend.BULLISH) reasons.push("Strong upward trend confirmed by EMAs");
        if (state.trend === MarketTrend.BEARISH) reasons.push("Strong downward trend confirmed by EMAs");
        if (divergence !== 'NONE') reasons.push(`RSI ${divergence} divergence detected`);
        if (state.isVolatilityExpanding) reasons.push("Volatility expansion (ATR) detected");
        if (state.hasBOS) reasons.push("Market structure shift (BOS) identified");
        if (state.hasLiquiditySweep) reasons.push("Liquidity sweep confirmed");
