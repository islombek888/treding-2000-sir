...
export class VisualAnalyzer {
    ...
    private chartPadding = 60;
    private candleWidth = 8;
    private candleSpacing = 2;

    constructor(private marketDataService: MarketDataService) {}

    public analyzeXAUUSD(): EntrySignal | null {
    ...
    public async generateChart(signal: EntrySignal): Promise<string> {
        const candles = this.marketDataService.getCandles('XAUUSD', '1m');
        const structure = this.analyzeMarketStructure(candles);
        const keyLevels = this.identifyKeyLevels(candles);
        const trend = this.analyzeTrend(candles);
    ...
    private drawChartBackground(ctx: CanvasRenderingContext2D): void {
        ...
        ctx.fillRect(
            this.chartPadding,
            this.chartPadding,
            this.canvasWidth - 2 * this.chartPadding,
            this.canvasHeight - 2 * this.chartPadding
        );
    }

    private drawGrid(ctx: CanvasRenderingContext2D, priceRange: { min: number; max: number }): void {
    ...
    private drawCandles(ctx: CanvasRenderingContext2D, candles: CandleData[], priceRange: { min: number; max: number }): void {
    ...
    private drawEMAs(ctx: CanvasRenderingContext2D, candles: CandleData[], priceRange: { min: number; max: number }, trend: TrendData): void {
        const chartWidth = this.canvasWidth - 2 * this.chartPadding;
        const chartHeight = this.canvasHeight - 2 * this.chartPadding;
        const visibleCandles = Math.min(100, candles.length);
        const startIndex = Math.max(0, candles.length - visibleCandles);
    ...
    private drawEMA(ctx: CanvasRenderingContext2D, emaData: number[], color: string, width: number, startIndex: number, chartWidth: number, chartHeight: number, priceRange: { min: number; max: number }): void {
        ...
        for (let i = 0; i < emaData.length; i++) {
            ...
            if (!emaValue || emaValue === 0) continue;
    ...
    private drawKeyLevels(ctx: CanvasRenderingContext2D, keyLevels: KeyLevel[], priceRange: { min: number; max: number }): void {
    ...
    private drawMarketStructure(ctx: CanvasRenderingContext2D, candles: CandleData[], structure: MarketStructure, priceRange: { min: number; max: number }): void {
        if (structure.bos) {
    ...
    private drawTrendline(ctx: CanvasRenderingContext2D, trendline: TrendData['trendline'], priceRange: { min: number; max: number }): void {
        if (!trendline) return;
    ...
    private drawEntrySignal(ctx: CanvasRenderingContext2D, signal: EntrySignal, candles: CandleData[], priceRange: { min: number; max: number }): void {
        const chartWidth = this.canvasWidth - 2 * this.chartPadding;
        const chartHeight = this.canvasHeight - 2 * this.chartPadding;
    ...
    private drawAnnotations(ctx: CanvasRenderingContext2D, signal: EntrySignal, structure: MarketStructure, trend: TrendData): void {
        ctx.fillStyle = '#ffffff';
    ...
    private calculatePriceRange(candles: CandleData[]): { min: number; max: number } {
    ...
    private identifyKeyLevels(candles: CandleData[]): KeyLevel[] {
        ...
        const priceTouches = new Map<number, number>();
    ...
    private analyzeTrend(candles: CandleData[]): TrendData {
    ...
    private generateEntrySignal(candles: CandleData[], structure: MarketStructure, keyLevels: KeyLevel[], trend: TrendData): EntrySignal | null {
    ...
    private getMainConfirmation(structure: MarketStructure, trend: TrendData, levels: KeyLevel[]): string {
        if (structure.bos) return 'Break of Structure Confirmed';
        if (trend.direction !== 'sideways') return 'Trend Continuation Expected';
        if (levels.length > 0) return 'Key Level Rejection';
