...
export class StructureAnalyzer {
    static detectStructure(candles: Candle[]): { type: 'HH' | 'HL' | 'LH' | 'LL' | 'NONE'; bos: boolean; sweep: boolean } {
        if (candles.length < 20) return { type: 'NONE', bos: false, sweep: false };
        ...
        let type: 'HH' | 'HL' | 'LH' | 'LL' | 'NONE' = 'NONE';
        ...
        if (currentHigh > prevHigh && currentLow > prevLow) type = 'HH';
        else if (currentHigh < prevHigh && currentLow < prevLow) type = 'LL';
        else if (currentHigh > prevHigh && currentLow <= prevLow) type = 'HL'; // Simplified
        else if (currentHigh <= prevHigh && currentLow < prevLow) type = 'LH'; // Simplified
