import { VisualAnalyzer, type EntrySignal } from './visualAnalyzer.js';
...
export class VisualAlertService {
    ...
    public async analyzeAndVisualize(): Promise<void> {
        const now = Date.now();
    ...
    private async sendVisualSignal(signal: EntrySignal, chartPath: string): Promise<void> {
        try {
            // Read chart image
...
⚠️ This is an advanced visual analysis with clear entry logic and risk management.
    ...
    public getLatestChart(): string | null {
