...
export class DecisionEngine {
     ...
     */
    static async decide(dataService, symbol, timeframe) {
        const primaryCandles = dataService.getCandles(symbol, timeframe);
        if (primaryCandles.length < 200)
            return null;
        ...
        if (effectivePhase === 'EXPANSION')
            strategyPrefix = 'Trend Following';
        else if (effectivePhase === 'CONSOLIDATION')
            strategyPrefix = 'Liquidity Sweep';
        const results = [];
        ...
        for (let i = 0; i < iterations; i++) {
            ...
            let higherTimeframeTrend = 'NEUTRAL';
            ...
            if (!isStrongSignal && ((higherTimeframeTrend === 'BEARISH' && result.strategy.includes('Buy')) ||
